...
def add_stock(stock_list):
      ...
      while option != "0":
          print("== ADD STOCK ==")
          print("")
          symbol = input("Insert a stock symbol: ")
          name = input("Type the company's name: ")
          shares = float(input("Insert a number of shares (ONLY NUMBERS): "))
...
def import_stock_csv(stock_list):
    print("\nAdd historical data to a tock in the stock data")
    print("Stock List:  [ ", end="")
    for stock in stock_list:
        print(stock.symbol, " ", end = "")
    print("]")
...
def display_report(stock_list):
    ...
    for stock in stock_list:
        ...
        if count > 0:
            ...
            print("High Price: ", highVolume)
            print("Average Volume: {:,.2f}".format(volume_total/count))
            print("Change in Proce: ${:,.2f}".format(priceChange))
            print("Profit/Loss: ${:,.2f}".format(priceChange * stock.shares))
        else:
            print("There is no daily history!")
        print("\n\n\n")
